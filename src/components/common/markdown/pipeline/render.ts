...
import { substituteBlocks } from './substitute'
import {
  COMPONENTS,
  escapeHtml,
  type HeadingEntry,
...
type RenderEnv = {
  ...
  /**
   * Component occurrences, in the order the parser met them.
   *
   * The renderer cannot call an Astro component (they are async and this runs
   * inside markdown-it's synchronous renderer rules), so each occurrence emits
   * a `<astro-md-block>` placeholder carrying an index into this table.
   * `substitute.ts` walks the finished HTML and swaps each placeholder for the
   * real component. See ../blocks/index.ts.
   */
  blocks: Array<{ name: string; ctx: RenderCtx }>
}
...
function createParser(): MarkdownIt {
  ...
  rules.directive_open = (tokens: any[], idx: number, _opts: any, rawEnv: any) => {
    ...
    const e = env(rawEnv)
    const at = e.blocks.length
    e.blocks.push({ name: info.spec.name, ctx: directiveCtx(tokens[idx], info, e) })
    // Recorded on the token so the matching close finds the same index even
    // when components nest.
    tokens[idx].meta = { ...info, blockIndex: at }
    return `<astro-md-block data-i="${at}">`
  }

  rules.directive_close = () => '</astro-md-block>'
 ...
 *
 * Async because component blocks are real Astro components rendered through
 * the container API. A document with no components never awaits anything past
 * the parse — `substituteBlocks` returns immediately when nothing was
 * recorded, which is the case for every project README and all but one of the
 * generated doc pages.
 */
export async function renderMarkdown(
  ...
  const withBlocks = await substituteBlocks(html, renderEnv.blocks)
  return postProcessHtml(withBlocks, urls, theme)
}
