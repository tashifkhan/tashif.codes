...
## Introduction
REST and webhook surface for the SuperSet Telegram Notification Bot. FastAPI routes, request/response shapes, Telegram command handling, rate limits, pagination, and how clients should call in.
...
- Commands:
 ...
 - /placement_year: Choose placement year (inline keyboard).
 - /stats: Placement statistics.
 - /noticestats: Notice statistics.
 - /web: Useful links.
 - /help: Command list.
 - /userstats: User statistics (admin).
 - Admin commands: /users, /boo, /fu, /scrapyyy, /s, /logs, /kill.
- Integration Points:
...
- Callback Mechanisms:
 ...
 - `/placement_year` also has a callback handler (`placement_year:`).
 - Admin commands gated by `ADMIN_TELEGRAM_USER_IDS`.
...
## Conclusion
REST for push, dispatch, and stats. Telegram bot for humans. Services are injected into the webhook app. There is no built-in rate limiter. Chunking and broadcast delays are the current brake. In production: protect the webhook, set VAPID keys, and watch the logs.
