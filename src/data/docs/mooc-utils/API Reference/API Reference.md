...
## Introduction
Notice Reminders exposes a REST API for auth, course search, announcements, subscriptions, and user profiles. Each endpoint below lists method, path, schemas, auth rules, and example payloads. The website client and the usual security notes (cookies, JWT, rate limits) live here too.
...
## Conclusion
Auth, courses, announcements, subscriptions, and profile routes share cookie sessions and ownership checks. Wire the website client with credentials included and treat 401/403 as a signal to refresh or re-login.
...
- Credentials
 - credentials: "include" so cookies are sent and received.
- Error Handling
