...
## Introduction
Vite bundles the React UI, Electron packages the desktop shell, and electron-builder produces platform installers.
...
E_PRELOAD["electron/src/electron/preload.cjs<br/>IPC bridge"]
...
- Base path: Relative base so assets resolve when loaded inside Electron.
...
- App ID and packaging files include `dist-react/**`, `src/electron/**`, `src/shared/**`, and `package.json`.
- Preload ships as `src/electron/preload.cjs` in `files`. There is no extraResources block.
- Targets:
 - macOS: dmg
...
```mermaid
flowchart TD
Cfg["electron-builder.json"] --> Files["files: dist-react, src/electron, src/shared"]
Cfg --> Icon["icon: desktopIcon.icns"]
Cfg --> Mac["mac: dmg"]
...
- Steps:
 - Checkout code
 - Setup Node.js 20+ with npm cache (Vite 8)
...
- Preload is packaged as `src/electron/preload.cjs` via the `files` glob.

```mermaid
graph LR
ViteOut["dist-react/index.html + assets"] --> ElectronLoad["Electron main loads index.html"]
ElectronLoad --> Renderer["React app renders"]
BuilderCfg["electron-builder.json files"] --> Packaged["Packaged app includes dist-react and src/electron"]
```
...
- electron-builder packages `dist-react` plus `src/electron` and `src/shared`.
...
- Keep the `files` glob tight so unused source stays out of the package.
...
- Distribution failures:
 - Validate electron-builder `files` globs and `desktopIcon.icns`.
...
Dev uses Vite's server; production loads static files from `dist-react`. Mixing those modes is a common "blank window" cause.
