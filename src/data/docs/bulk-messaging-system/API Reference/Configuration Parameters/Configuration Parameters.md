...
## Introduction
Settings that control WhatsApp, Gmail, and SMTP. Most of this lives in env vars, electron-store, and a few hardcoded client options.
...
Notes:
- The application deletes cached .wwebjs_cache and .wwebjs_auth directories on startup and logout so sessions start clean.
...
Wrong values here usually show up as auth failures or silent no-ops. Change one service at a time and verify with a single test send.
