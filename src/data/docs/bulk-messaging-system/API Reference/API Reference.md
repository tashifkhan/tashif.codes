...
## Introduction
Public surfaces the desktop app exposes: Electron IPC handlers, the Flask contact API, and the knobs those services read from env and config.
...
If you are wiring a new UI action, start from the IPC handler names here, then follow the payload into the matching Gmail, SMTP, WhatsApp, or Flask code.
