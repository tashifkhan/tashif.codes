...
The TalentSync Prisma schema (`public`) plus the SQLModel tables Alembic owns under `talentsync_backend`.
...
- Models: `User` (role column, not a Role table), `Account`, `Session` (hashed refresh, backend-owned), resumes, analysis, leftover bulk-upload/recruiter rows, outreach, interviews, LLM config, billing.
...
- Generation process: Executed during build via npm script, which generates a type-safe client
